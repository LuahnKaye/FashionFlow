...
@roteador.get("/{id_pedido}", response_model=PedidoResposta)
def obter_pedido(id_pedido: int, banco: Session = Depends(obter_banco), dados_token: dict = Depends(verificar_token_acesso)):
    """
    Retorna os detalhes de um pedido para acompanhamento de status.
    """
