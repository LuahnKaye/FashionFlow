...
aplicativo_fastapi = FastAPI(title="AtivoSAGA - Servico de Estoque")

@aplicativo_fastapi.on_event("startup")
...
@aplicativo_fastapi.post("/definir")
def definir_estoque(item: ItemEstoque, banco: Session = Depends(obter_banco)):
    """
    Define a quantidade absoluta de um produto no estoque.
    """
    existente = banco.query(Estoque).filter(Estoque.id_produto == item.id_produto).first()
    if existente:
        existente.quantidade_disponivel = item.quantidade
        existente.quantidade_reservada = 0
    else:
        novo_item = Estoque(id_produto=item.id_produto, quantidade_disponivel=item.quantidade, quantidade_reservada=0)
        banco.add(novo_item)
    
    banco.commit()
    return {"mensagem": f"Estoque definido para {item.quantidade} para o produto {item.id_produto}"}
